...
ENV["GKSwstype"] = "100" # i added this -> it worked for adroit-vis!
...
for i in 1:n_exp
    ...
    prob_trueode = ODEProblem(trueODEfunc, u0, tspan, k, kwargshandle=KeywordArgSilent)
    ode_data = Array(solve(prob_trueode, alg, saveat=tsteps, kwargshandle=KeywordArgSilent))[1:end - 1, :]
...
u0 = u0_list[1, :];
prob = ODEProblem(crnn, u0, tspan, saveat=tsteps, atol=atol, rtol=rtol, kwargshandle=KeywordArgSilent)

sense = BacksolveAdjoint(checkpointing=true; autojacvec=ZygoteVJP());
function predict_neuralode(u0, p)
    global w_in, w_b, w_out = p2vec(p)
    pred = clamp.(Array(solve(prob, alg, u0=u0, p=p, sensalg=sense, kwargshandle=KeywordArgSilent)), -ub, ub)
    return pred
end
